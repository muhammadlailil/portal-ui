...
<li role="menuitemcheckbox" aria-disabled="{{ $disabled ? 'true' : 'false' }}" aria-label="{{ $slot }}"
        ...
        triggerClick() {
          if($el.getAttribute('aria-disabled')=='false'){
               if (this.checked.includes(this.value)) {
                    this.checked = this.checked.filter((row) => row != this.value)
               } else {
    ...
    <span class="absolute left-2 flex h-3.5 w-3.5 items-center justify-center" x-show="checked.includes(value)" x-cloak>
        <x-tabler-check class="w-4 h-4" />
    </span>
    <template x-if="checked.includes(value)">
